...
#include "asm/unistd_64.h"      // TODO: arch dependent? maybe '#include <unistd.h>' instead?
#include "sys/wait.h"
...
#define WRITE_LOG(fmt, ...) ({	\
	fprintf(stderr, "LOGMSG: - " fmt "\n", ##__VA_ARGS__);	\
})

#define WRITE_ERR(tag, fmt, ...) ({  \
	fprintf(stderr, "ericsu - %s failed. " fmt "\n", tag, ##__VA_ARGS__);    \
})
...
#define ASSERT_OK(res, name) ({     \
	...
	bool ___ok = ___res == 0;       \
	_CHECK(!___ok, \
			"ericsu - %s failed. unexpected error: %lld (errno %d)\n",  \
			 name, ___res, errno);  \
	___ok;                          \
})
...
#define ASSERT_GT(actual, expected, name) ({    \
	...
	_CHECK(!___ok,  \
			"ericsu - unexpected %s: actual %lld <= expected %lld (errno %d)\n",   \
...
#define ASSERT_ERR_PTR(ptr, name) ({    \
	...
	bool ___ok = ___err != 0;           \
	_CHECK(!___ok,  \
			"ericsu - %s unexpected pointer(%p): %d\n", name, ___res, ___err);  \
	___ok;      \
})
...
static int child(int sock_fd, struct bpffs_opts *bpffs_opts, child_callback_fn callback)
{
...
static int userns_map_create(int mnt_fd)
{
	...
	map_fd = bpf_map_create(BPF_MAP_TYPE_STACK, "userns_map_create", 0, 8, 1, &map_opts);
...
static int userns_btf_load(int mnt_fd)
{
	...
	int err = -EINVAL, token_fd = -1, btf_fd = -1;
	const void *raw_btf_data;
	struct btf *btf = NULL;
	__u32 raw_btf_size;
	
	token_fd = bpf_token_create(mnt_fd, NULL);
	if (!ASSERT_GT(token_fd, 0, "user_btf_load/token_create"))
		goto cleanup;

	btf = btf__new_empty();
	if (!ASSERT_OK_PTR(btf, "user_btf_load/empty_btf"))
		goto cleanup;

	if (!ASSERT_GT(btf__add_int(btf, "int", 4, 0), 0, "int_type"))
		goto cleanup;

	raw_btf_data = btf__raw_data(btf, &raw_btf_size);
	if (!ASSERT_OK_PTR(raw_btf_data, "user_btf_load/raw_btf_data"))
		goto cleanup;

	btf_opts.btf_flags = BPF_F_TOKEN_FD;
	btf_opts.token_fd = token_fd;
	btf_fd = bpf_btf_load(raw_btf_data, raw_btf_size, &btf_opts);
	if (!ASSERT_GT(btf_fd, 0, "user_btf_load/bpf_btf_load"))
		goto cleanup;
...
int main(int argc, char *argv[])
{
	...
	if (argc != 2) {
		WRITE_ERR("main", "Unexpected arg count passed to token test. "
			"Expected: 1, Actual: %d", argc);
		err = -1;
		goto cleanup;
	}

	target_test = argv[1];
	if (strcmp(target_test, "map_token") == 0) {
		...
		err = subtest(&opts, userns_prog_load);
	} else if (strcmp(target_test, "obj_priv_map") == 0) {
		struct bpffs_opts opts = {
			.cmds = bit(BPF_MAP_CREATE),
			.maps = bit(BPF_MAP_TYPE_QUEUE),
		};
		err = subtest(&opts, userns_obj_priv_map);
	} else if (strcmp(target_test, "obj_priv_prog") == 0) {

	} else if (strcmp(target_test, "obj_priv_btf_fail") == 0) {

	} else if (strcmp(target_test, "obj_priv_btf_success") == 0) {

	} else if (strcmp(target_test, "obj_priv_implicit_token") == 0) {

	} else if (strcmp(target_test, "obj_priv_implicit_token_envvar") == 0) {

	} else {
		WRITE_ERR("main", "Unexpected test %s", target_test);
		goto cleanup;
	}
